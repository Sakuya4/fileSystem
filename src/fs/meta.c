...
int meta_load(void)
{
    ...
    uint32_t entry_bytes  = hdr.entry_count * sizeof(meta_entry_t);
    uint32_t entry_blocks = (entry_bytes + BLOCK_SIZE - 1) / BLOCK_SIZE;
    ...
    if (block_read(blkno, buf) != 0) return -1;

    /* ========= 第一輪：建 inode + dentry，不掛 ========= */
    for (uint32_t i = 0; i < hdr.entry_count; i++) {
        ...
        struct inode *ino = calloc(1, sizeof(struct inode));
        ino->i_type = e.type;
        ino->i_size = e.size;
        ino->i_mode = (e.type == FS_INODE_DIR) ? (FS_IFDIR | 0755)
                                               : (FS_IFREG | 0644);
        ...
        for (int k = 0; k < DIRECT_BLOCKS; k++)
        {
          ...
          if(e.blocks[k]>=0)
          {
            block_reserve(e.blocks[k]);
          }
        }
        struct dentry *dent = calloc(1, sizeof(struct dentry));
    ...
    /* ========= 第二輪：照 parent index 掛回樹 ========= */
    for (uint32_t i = 0; i < hdr.entry_count; i++) {
        if (!dent_list[i]) continue;

        int p = entry_list[i].parent;
        if (p < 0)
